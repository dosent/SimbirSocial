...
/**
 * REST controller for managing {@link com.simbirsoft.simbirsocial.domain.Events}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Events> result = eventsRepository
            ...
            .map(existingEvents -> {
                ...
                if (events.getLatitude() != null) {
                    existingEvents.setLatitude(events.getLatitude());
                }
                if (events.getLongitude() != null) {
                    existingEvents.setLongitude(events.getLongitude());
                }
                if (events.getUser() != null) {
                    existingEvents.setUser(events.getUser());
                }
                if (events.getPubish() != null) {
                    existingEvents.setPubish(events.getPubish());
                }
                if (events.getCategory() != null) {
                    existingEvents.setCategory(events.getCategory());
                }
