...
import com.simbirsoft.simbirsocial.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.simbirsoft.simbirsocial.domain.Files}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class FilesResource {
     ...
     */
    @PostMapping("/files")
    public ResponseEntity<Files> createFiles(@Valid @RequestBody Files files) throws URISyntaxException {
     ...
     */
    @PutMapping("/files/{id}")
    public ResponseEntity<Files> updateFiles(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Files files)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, files.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Files> partialUpdateFiles(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Files files
    ) throws URISyntaxException {
        ...
        Optional<Files> result = filesRepository
            ...
            .map(existingFiles -> {
                ...
                if (files.getDescription() != null) {
                    existingFiles.setDescription(files.getDescription());
                }
                if (files.getPath() != null) {
                    existingFiles.setPath(files.getPath());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, files.getId().toString())
        );
    }
